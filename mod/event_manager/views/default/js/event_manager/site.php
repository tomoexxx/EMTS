...
(function(){
    ...
    /* Add(Copy) Tani 2013.06.27 */
    // toggle drop down menu
    $(".event_manager_tool_actions").live("click", function(tool){
        if($(this).next().is(":hidden")){
            // only needed if the current menu is already dropped down
	    	$("body > .event_manager_tool_actions_drop_down").remove();
			$("body").append($(this).next().clone());
			css_top = $(this).offset().top + $(this).height(); 
			css_left = $(this).offset().left; 
			$("body > .event_manager_tool_actions_drop_down").css({top: css_top, left: css_left}).show();;
        }
