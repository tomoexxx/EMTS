...
function groups_handle_all_page() {
	...
	$metastring = get_data_row("SELECT id FROM {$db_prefix}metastrings WHERE string = 'interests'");
 ...
 * List contest groups
 */
function groups_handle_contest_page() {
	...
	$filter = elgg_view('groups/contest_sort_menu', array('selected' => $selected_tab));
