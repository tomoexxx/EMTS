...
function detectmobile(){
	...
	if(preg_match('/(docomo|up\.browser|softbank|vodafone|j-phone|smot|willcom|emobile)/i', $_SERVER['HTTP_USER_AGENT'])) {
		return true;
	} else {
		return false;
	}
...
function mobilize_setup_handler() {
	...
	elgg_register_menu_item('footer', array(
		...
		'text' => elgg_echo('mobilize:copyright') . $year . elgg_echo(' Produced by JoinWith'),
		'priority' => 1,
		'section' => 'alt',
	));
	
	// Extend footer with elgg link
	/* Delete Tani 2013.06.19 */
	/*
	$href = "http://elgg.org";
	elgg_register_menu_item('footer', array(
		'name' => 'elgg',
